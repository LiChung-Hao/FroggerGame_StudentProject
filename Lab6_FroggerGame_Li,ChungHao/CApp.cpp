...
//==============================================================================
bool CApp::check_onLog(SDL_Rect A, SDL_Rect B)
{
...
bool CApp::check_LogCollision(SDL_Rect A, SDL_Rect B)
{
    // The sides of the rectangles
    int leftA, leftB;
    int rightA, rightB;
...
bool CApp::loadMedia()
{
    ...
    //Load sound effects
    gScratch = Mix_LoadWAV( "hit.mp3" );
    if( gScratch == NULL ){
        printf( "Failed to load scratch sound effect! SDL_mixer Error: %s\n", Mix_GetError() );
        Running = false;
    }

    gScratch1 = Mix_LoadWAV("WIN.WAV");
    if (gScratch1 == NULL) {
        printf("Failed to load scratch sound effect! SDL_mixer Error: %s\n", Mix_GetError());
        Running = false;
    }

    gScratch2 = Mix_LoadWAV("Pew.mp3");
    if (gScratch2 == NULL) {
        printf("Failed to load scratch sound effect! SDL_mixer Error: %s\n", Mix_GetError());
        Running = false;
    }
